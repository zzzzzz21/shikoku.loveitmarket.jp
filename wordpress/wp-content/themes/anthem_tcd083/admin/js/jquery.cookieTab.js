		...
		return this.each(function(){
				...
				$tabMenu.find('a').each(function(value){ //value : array number of clicked anchor-element
					$(this).click(function(){ //action for tab-menu click
						...
						if(tabValue != value){ //when clicking current tab, move to link page.
							tabValueLast2 = tabValueLast;
							tabValueLast = tabValue;
							tabValue = value;
							tabDisplay();
							return false;
						}
			...
			function tabDisplay(){
				var triggerArgs = {
					tabDisplay: true,
					tabValue: tabValue,
					tabValueLast: tabValueLast,
					$beforeTabPanel: $tabPanel.eq(tabValueLast),
					$afterTabPanel: $tabPanel.eq(tabValue)
				};
				$unit.trigger('jctBeforeTabDisplay', [triggerArgs]);
				if (!triggerArgs.tabDisplay) {
					tabValue = tabValueLast;
					tabValueLast = tabValueLast2;
					tabValueLast2 = -1;
					if (options.cookie) {
						setCookie($cookieName, tabValue, options.cookie.expires, location.hostname, options.cookie.path, options.cookie.secure);
					}
					return;
				}
				...
				$unit.trigger('jctAfterTabDisplay');
			}
