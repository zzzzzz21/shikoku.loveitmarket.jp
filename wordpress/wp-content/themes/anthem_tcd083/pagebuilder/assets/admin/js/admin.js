jQuery(document).ready(function($){
	...
	if ($('.block-editor__container').length) {
		...
		wp.domReady(function(){
			setTimeout(function(){
				// WordPress 6.0
				if (!$editor.find('.edit-post-more-menu').length) {
					be_selectors = {
				...
				$editor.on('click', be_selectors.more_menu_button, function(){
					setTimeout(function(){
						// ポップメニューが開いてなければ終了
						if (!$editor.find(be_selectors.more_menu_content).length) return;
				...
				// エディター選択ボタンクリック
				$('#editor').on('click', be_selectors.editor_button, function(){
